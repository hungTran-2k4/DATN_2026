...
{
    Task<(IEnumerable<Product> Items, int Total)> GetPagedAsync(Guid? shopId = null, string? search = null, FilterDescriptor? filter = null, ProductStatus? status = null, int page = 1, int pageSize = 20, CancellationToken cancellationToken = default);
